...
//Read input informations
  ...
  ReadInput >> nbins;
  cout << "Number of bins for gofr = " << nbins << endl;
  bin_size = 0.5*box/double(nbins);
  cout << "Size of bins for gofr = " << bin_size << endl;
  ...
  ReadInput >> nstart;    //#steps necessary to reach equilibration
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
  iw = 4; //Pressure
  ig = 5; //Pair distribution function
...
// distance i-j in pbc
  ...
  walker[iw]  = rho * walker[it]  + 48.0/(3*vol) * w; // Pressure 
...
void ComputeTail()
{
  ...
  double correction = 1.0/9.0 * 1.0/pow(rcut,9) - 1.0/3.0 * 1.0/pow(rcut,3) ;
  vtail = 8.0 * pi * rho * correction;
  ptail = vtail * 4.0 * rho;
  //Print tail corrections
  cout << "Computing tail corrections to be added to potential energy and pressure..." << endl;
  cout << "Tail correction on potential energy = " << vtail << endl;
  cout << "Tail correction on pressure         = " << ptail << endl;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    stima_pot += vtail; //Tail correction of the potential per particle
    glob_av[iv] += stima_pot;
    glob_av2[iv] += stima_pot*stima_pot;
    err_pot=Error(glob_av[iv],glob_av2[iv],iblk);
    
    stima_kin = blk_av[ik]/blk_norm/(double)npart; //Kinetic energy per particle
    ...
    stima_press += ptail; //Tail correction of pressure 
    glob_av[iw] += stima_press;
    glob_av2[iw] += stima_press*stima_press;
    err_press=Error(glob_av[iw],glob_av2[iw],iblk);

    for(int k=0; k<nbins; k++){ //average gofr in each bin
      double V_r_dr = 4.0/3.0 * pi * ( pow((k+1)*bin_size,3) - pow(k*bin_size,3) );
      //stima_gofr[k] = blk_av[ig+k]/blk_norm/(double(npart)*rho*V_r_dr);  //Radial distribution function
      //glob_av[ig+k] += stima_gofr[k];
      //glob_av2[ig+k] += stima_gofr[k]*stima_gofr[k];
      //err_gofr[k]=Error(glob_av[ig+k],glob_av2[ig+k],iblk); //I will compute the error while printing
      stima_gofr = blk_av[ig+k]/blk_norm/(double(npart)*rho*V_r_dr);  //Radial distribution function
      glob_av[ig+k] += stima_gofr;
      glob_av2[ig+k] += stima_gofr*stima_gofr;
      err_gofr=Error(glob_av[ig+k],glob_av2[ig+k],iblk);
      // Printing radial distribution function for each bin
      Gofr << setw(wd) << (k+0.5)*bin_size << setw(wd) << stima_gofr << setw(wd) << glob_av[ig+k]/(double)iblk << setw(wd) << err_gofr << endl;
      if(iblk==nblk){   // if in the final block also in a separate file
        Gofr.close();
        Gofr.open("output_gofr_final.dat",ios::app);
        Gofr << setw(wd) << (k+0.5)*bin_size << setw(wd) << glob_av[ig+k]/(double)iblk << setw(wd) << err_gofr << endl;
      }
    }
...
//    for(int k=0; k<nbins; k++)
//      Gofr << setw(wd) << (k+0.5)*bin_size << setw(wd) << stima_gofr[k] << setw(wd) << glob_av[ig+k]/(double)iblk << setw(wd) << err_gofr[k] << endl;
//    if(iblk==nblk){
//      Gofr.close();
//      Gofr.open("output_gofr_final.dat",ios::app);
//      for(int k=0; k<nbins; k++)
//      Gofr << setw(wd) << (k+0.5)*bin_size << setw(wd) << glob_av[ig+k]/(double)iblk << setw(wd) << err_gofr[k] << endl;
//    }
